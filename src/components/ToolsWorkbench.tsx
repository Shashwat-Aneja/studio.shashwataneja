...
const toolCopy=[
 ...
 {id:"health",n:"10",title:"Website Health Scanner",desc:"Run a transparent preflight before a deeper technical audit."}
] as const;
 ...
 return <section className="tool-lab" id="tool-lab">
  ...
  <div className="tool-lab-layout">
   ...
   <div className="tool-lab-stage" key={active}>
    ...
    {active==="health"&&<div className="lab-card health-tool"><label><span>Website URL</span><input value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://yourwebsite.com"/></label>{url&&<div className="health-grid">{[["Secure URL",url.startsWith("https://"),"HTTPS is present in the address."],["Recognisable host",/^https?:\\/\\/[^\\/]+$/.test(url),"A normal site host is detected."],["Canonical path",!url.endsWith("/"),"A clean root URL is easier to audit consistently."],["Ready for deeper audit",url.startsWith("http"),"A real scanner can later inspect public technical signals server-side."]].map(([name,ok,note])=><div key={name as string}><span className={ok?"health-ok":"health-warn"}>{ok?"PASS":"CHECK"}</span><strong>{name}</strong><small>{note}</small></div>)}</div>}<p className="health-disclaimer">This first-pass tool intentionally does not pretend to fetch or grade a live website from the browser. The production scanner should perform public-signal checks server-side.</p><button className="hero-cta" onClick={()=>onStart("growth")}>Request a full audit route ↗</button></div>}
