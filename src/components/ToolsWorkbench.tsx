 ...
 return <section className="tool-lab" id="tool-lab">
  ...
  <div className="tool-lab-layout">
   ...
   <div className="tool-lab-stage" key={active}>
    ...
    {active==="timeline"&&<div className="lab-card timeline-tool">{phases.map((p,i)=><div className="timeline-row" key={p.name}><span className="mono">0{i+1}</span><div><strong>{p.name}</strong><small>{p.note}</small></div><b>{p.duration}d</b><i style={{"--phase":p.duration} as React.CSSProperties}/></div>)}</div>}
    {active==="presence"&&<div className="lab-card score-tool"><div className="score-ring" style={{"--score":presence} as React.CSSProperties}><strong>{presence}</strong><span>/100</span></div><div className="score-inputs">{["Presence","Discovery","Trust","Conversion","Performance","Infrastructure"].map((name,i)=><label key={name}><span>{name}</span><input type="range" min="0" max="10" value={scores[i]} onChange={e=>setScores(s=>s.map((v,j)=>j===i?Number(e.target.value):v))}/><b>{scores[i]}</b></label>)}</div><p>Self-assessment only. A real scanner can later combine public technical signals with these dimensions.</p></div>}
